...
- No side effects or domain semantics inside `stably-ts`
...
### 2.1 Core Principle
`stably-ts` must depend **only on packages that do not introduce:**
...
- Add dependencies to `stably-ts` that could introduce side effects
