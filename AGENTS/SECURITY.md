...
Nothing in `stably-ts` depends on or interacts with external systems.
...
Agents MUST NOT introduce into `stably-ts`:
...
Examples (future `stably-examples/`, a Python `stably-mcp-server`, etc.) may include I/O and network operations.

However:

- Agents MUST keep those dependencies isolated from `stably-ts`
